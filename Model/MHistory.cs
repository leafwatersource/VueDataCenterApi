...
namespace WebApiNew.Model
{
    public class MHistory
    {
        ...
        public JObject GetUserLog(string PageSize, string CurPage, string filte)
        {
            JObject obj = AppSetting.TableFileds.SelectToken("HistoryTableFiled").ToObject<JObject>();
            string sqlStr = "";
            DataTable table = new DataTable();
            int total = GetCountLog();
            int minPmuid = 0;
            if (Convert.ToInt32(CurPage) > 1)
            {
                minPmuid = LastPmuid((Convert.ToInt32(CurPage) - 1) * Convert.ToInt32(PageSize));
            }
            ...
            {
                cmd.CommandText = "SELECT top " + PageSize + ' ' + sqlStr + " from wapMesEventRec  order by EventTime desc";

            }
            else
            {
                cmd.CommandText = "SELECT top " + PageSize + ' ' + sqlStr + " from wapMesEventRec where pmuid < '" + minPmuid + "' order by EventTime desc";
        ...
        /// <returns></returns>
        public int LastPmuid(int end)
        {
            DataTable table = new DataTable();
            SqlCommand cmd = PmConnections.SchCmd();
            int min;
            cmd.CommandText = "select top " + end + " PMUID from wapMesEventRec order by EventTime desc";
